      ...
      <div>
        {service.img ? (
          <div className="relative aspect-[4/3] w-full rounded-xl overflow-hidden ring-1 ring-zinc-100">
            <Image src={service.img} alt={service.label} fill className="object-cover" />
          </div>
        ) : null}
      </div>

      <div className="space-y-4">
        <h2 className="text-xl font-semibold text-[var(--dark-text)]">Como funciona</h2>
        <p className="text-zinc-700">
          Detalhe aqui o processo, materiais, prazos, garantias e diferenciais do serviço.
        </p>

        <div className="pt-2">
          <Button asChild className="rounded-full bg-[var(--secondary)] hover:bg-[color:var(--primary)]">
...
export function generateStaticParams() {
  ...
  for (const c of SERVICE_CATEGORIES) for (const s of c.children)
