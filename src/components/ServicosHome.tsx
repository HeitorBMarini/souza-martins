...
import imgPlaceholder from "@/components/imgs/servicos/servico.png"; // fallback

export default function ServicosHome() {
  // monta os “tiles” da home usando as categorias
  const tiles = SERVICE_CATEGORIES.map((cat) => ({
    href: `/servicos/${cat.category}`,
    label: cat.label,
    desc: cat.desc,
    img: cat.children[0]?.img ?? imgPlaceholder, // usa a 1ª imagem da categoria (ou um placeholder)
  }));
            ...
            <Link key={s.href} href={s.href} className="block">
              <article className="group overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-zinc-100 transition hover:shadow-md">
                ...
                <div className="p-4">
                  ...
                  <div className="mt-4">
                    <Button size="sm" className="rounded-full bg-[var(--secondary)] uppercase text-white hover:bg-[color:var(--primary)]">
                      Saiba mais
                    </Button>
                  </div>
                </div>
